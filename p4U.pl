...
sub afficherDamier 
 {
  my $pdamier = shift;   # il faut passer le tableau sous forme de "pointeur"
  my $dimX    = shift;
  my $dimY    = shift;
  ...
  for ($y=0; $y < $dimY; $y++)
   {
    ...
    for ($x=0; $x<$dimX; $x++) { print "+-"; }
                                 print "+\n";

    for ($x=0; $x<$dimX; $x++) { print "|".@{$pdamier}[$x*$dimY+$y]; }
                                 print "|\n";
   }
 ...
 #                                                                #
 #  - une valeur comprise en 6 et 0 correspondant à une case vide #
